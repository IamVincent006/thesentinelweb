...
class Transaction extends Model {
    ...
    public function lasttransaction_model(){
   ...
   /* public function update_transaction($parkID,$exitlogid,$exitdate,$process) {
    ...
    public function Monitoring() {
        ...
        $now = new DateTime();
        $date = $now->modify('-7 day')->format('Y-m-d 00:00:00');


        $condition = new QueryGroup();   
        $condition->and_query(new QueryField($this->tableName.".entrydate",">=",$date));
        $condition->or_query(new QueryField($this->tableName.".process","=","0"));
    ...
    public function transaction_model1($datemodified){
        $condition = new QueryGroup(); 
        $condition->and_query(new QueryField("process","=","1")); 
    ...
    public function transaction_getdate_model($terminal,$todate){
        ...
        $condition->and_query(new QueryField($this->tbl_payment.".receiptnum","<>", ""));
    ...
    public function bir_getdate_model($terminal,$from_date,$todate){
    ...
    public function transaction_oldreceipt_model($terminal){
        ...
        $select->add_fields(array("
                    ...
                    max($this->tbl_payment.receiptnum) as oldmax_or,                    
    ...
    public function transaction_groupbydate_modelz($terminal,$from_date,$to_date){
    ...
    public function records_byteller_model($terminal,$from_date,$to_date){
        ...
        $select->add_join(new Join($this->tbl_payment, new QueryField($this->tableName.".park_id", "=", $this->tbl_payment.".park_id"),"RIGHT"));
        $select->set_condition($condition);
        $select->add_sort("$this->tbl_payment.paymentdate");
        $checkTransList =   $this->select($select);
