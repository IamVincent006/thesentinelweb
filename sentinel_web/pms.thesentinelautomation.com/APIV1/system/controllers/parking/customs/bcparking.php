...
class BCParking {
	...
	public function update_parkingid()
	{
		...
		/*$terminaldata   = array(
     	...
     	$terminaldata   = array(
             "termparkid"  =>  (int)$terminalDetails[0]["termparkid"] + 1,                    
     	);			
	...
	public function transaction_groupbydatez()
	{
		$terminal = $_REQUEST['terminal'];
		$from_date = $_REQUEST['from_date'];
		$to_date = $_REQUEST['to_date'];
		...
		$result = $this->transaction->transaction_groupbydate_modelz($terminal,$from_date,$to_date);
...
/*######SYNCHING DATABASE###################################*/
	...
	public function getsyncrate(){	
		...
		if(isset($_REQUEST['date_modified']))
			$datemodified = $_REQUEST['date_modified'];
		else
			$datemodified= "0000-00-00 00:00:00";

		$result = $this->rate->get_rates_model1($datemodified);
		echo json_encode($result);
		//if(count($result)>0)
		//	echo json_encode($result[0]);
		//else
	...
	public function getlastsyncrate(){	

		$result = $this->rate->lastrate_model();
		//echo json_encode($result);
		$response = array(
			'status' => "200",
			'message' => "OK",
	...
	public function getsyncvoucher(){	
		...
		if(isset($_REQUEST['lastvoucherid']))
			$lastvoucherid = $_REQUEST['lastvoucherid'];
		else
			$lastvoucherid= "0";



		$result = $this->voucher->voucher_model($lastvoucherid);
		echo json_encode($result);

			
	}
	//new
	public function getsyncvoucher1(){	
		...
		$result = $this->voucher->voucher_model1($datemodified);
		echo json_encode($result);

			
	}

	public function getsynctransaction1(){	
		...
		$result = $this->transaction->transaction_model1($datemodified);
		echo json_encode($result);

			
	}

	public function getsynctellerlog1(){	
		...
		$result = $this->tellerlog->tellerlog_model1($datemodified);
		echo json_encode($result);

			
	}

	public function getsyncpayment1(){	
		...
		$result = $this->payment->payment_model1($datemodified);
		echo json_encode($result);

			
	}

	public function getlastsynctransaction(){		
		$result = $this->transaction->lasttransaction_model();
		//echo json_encode($result);
		$response = array(
			'status' => "200",
			'message' => "OK",
	...
	public function getlastsynctellerlog(){		
		$result = $this->tellerlog->lasttellerlog_model();
		//echo json_encode($result);
		$response = array(
			'status' => "200",
			'message' => "OK",
	...
	public function getlastsyncpayment(){		
		$result = $this->payment->lastpayment_model();
		//echo json_encode($result);
		$response = array(
			'status' => "200",
			'message' => "OK",
	...
	public function getlastsyncvoucher(){		
		$result = $this->voucher->lastvoucher_model();
		//echo json_encode($result);
		$response = array(
			'status' => "200",
			'message' => "OK",
	...
	public function transaction_total_xreading() {
		...
		//if(!isset($result[0]["min_void"]))
		$result[0]["oldmin_void"] = $oldreceipt[0]["oldmin_void"];
		//if(!isset($result[0]["max_void"]))
		$result[0]["oldmax_void"] = $oldreceipt[0]["oldmax_void"];
		if(!isset($result[0]["min_refund"]))
			$result[0]["min_refund"] = $oldreceipt[0]["oldmin_refund"];
		if(!isset($result[0]["max_refund"]))
			$result[0]["max_refund"] = $oldreceipt[0]["oldmax_refund"];
	...
	public function birdaterange() {
		$terminal = $_REQUEST['terminal'];
		$from_date = $_REQUEST['from_date'];
		$to_date = $_REQUEST['to_date'];

		$result = $this->transaction->bir_getdate_model($terminal,$from_date,$to_date);
	...
	public function transaction_total_zreading() {
		...
		$result[0]["oldmax_void"] = $oldreceipt[0]["oldmax_void"];
		$result[0]["oldmax_or"] = $oldreceipt[0]["oldmax_or"];
